...
The kernel package currently uses `pkgver=6.18` and `pkgrel=74`.
...
r50-r74 packages installed; that would make future packages look like
...
The r74 package also stages Android MT6878 connectivity modules from the Nothing
...
2. Validate r74 MT6631 vendor module loading on device: conninfra, WMT, Wi-Fi
   and Bluetooth.
