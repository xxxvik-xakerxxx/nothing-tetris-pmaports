...
| `0062-drm-mediatek-program-MT6878-DSC.patch` | Programs MT6878 DSC geometry and PPS. The paired working r132/LK reference matches r144 for every captured DSC data register except `DSC_MODE`: LK retains `0x10001`, while r144 writes `0x1`. The r145 candidate sets the parameter-load bit during initial configuration; late live injection was not sufficient evidence, so clean-boot validation remains required. |
...
| `0085-drm-mediatek-start-MT6878-pipeline-before-mutex.patch` | Restricts a vendor-ordering quirk to `mt6878-mmsys0`: configure and start OVL/DSC/DSI plus the active scanout plane before enabling mutex/SOF. This matches the B4.1 DRM v2 transaction boundary. Clean r145 and reversible live probes show the persistent first-frame stall cannot be repaired by late DSC reset or OVL trigger writes, so r146 tests the remaining cold-start boundary. |
