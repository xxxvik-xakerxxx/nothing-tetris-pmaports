...
| `1201-vendor-sensorhub-reject-mismatched-list-reply.patch.vendor` | Tightens the Nothing OS 4.1 sensor-list reply guard so any sequence, sensor type or command mismatch is rejected before consuming the shared-memory write position. The pinned-source gate reproduces the old six false accepts and proves the packaged predicate over eight combinations, all 65536 sequence pairs and two operator mutations. It adds no SCP startup, DT, autoload or sensor publication. |
