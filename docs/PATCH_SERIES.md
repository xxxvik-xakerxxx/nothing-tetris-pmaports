...
The kernel package currently uses `pkgver=6.18` and `pkgrel=71`.
...
r50-r71 packages installed; that would make future packages look like
...
The r71 package also stages Android MT6878 connectivity modules from the Nothing
...
2. Validate r71 MT6631 vendor module loading on device: conninfra, WMT, Wi-Fi
   and Bluetooth.
