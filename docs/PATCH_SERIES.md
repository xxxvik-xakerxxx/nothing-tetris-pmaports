...
| `0062-drm-mediatek-program-MT6878-DSC.patch` | Programs MT6878 DSC geometry and PPS. The paired working r132/LK reference matched r144 for every captured DSC data register except `DSC_MODE`: LK retained `0x10001`, while r144 wrote `0x1`. Clean r145 sets that parameter-load bit during initial configuration but still stalls, proving it is necessary vendor parity rather than the complete fix. |
...
| `0090-power-supply-mt6375-bc12-lifecycle-compile-only.patch` | Compile-only BC1.2 lifecycle model and host tests covering bounded DP/DM release, retryable partial cleanup, role-granted ownership and current attach-state reconciliation. CI rejects DT, IRQ, PHY, regmap, module, packaging and autoload wiring; production USB behavior remains unchanged. |
...
| `0050-media-i2c-imx882-honor-active-low-reset.patch` | Corrects the compile-only IMX882 probe to assert the active-low reset while rails settle and during shutdown, then release it only after MCLK is stable. The host gate covers success and all 11 power-on failure points; the sensor remains disabled in the shipped DT and config. |
...
| `patches/sensors/0001-sensor-list-reject-mismatched-reply.patch.vendor` | Demoted host-side candidate after the r11 clean-boot regression. It tightens the Nothing OS 4.1 sensor-list reply guard so any sequence, sensor type or command mismatch is rejected before consuming the shared-memory write position. The pinned-source gate reproduces the old six false accepts and proves the candidate over eight combinations, all 65536 sequence pairs and two operator mutations. It is not packaged as `1201` in the r12 recovery image and adds no SCP startup, DT, autoload or sensor publication. |
...
| `0093-vendor-scp-validate-region-info.patch.vendor` | Adds a compile-only, fail-closed structural gate before SCP recovery: the known region-info layout must fit in mapped TCM, its reported size must cover every consumed field, loader/firmware ranges must be non-empty and non-wrapping, the loader must fit in declared SCP TCM, and a requested DRAM range must be valid. Errors leave `scp.ko` unloaded. This does not authenticate or select `scp_a`/`scp_b`, prove carveout ownership, or enable runtime DT. |
| `0094-vendor-scp-validate-dram-recovery-span.patch.vendor` | Extends the fail-closed SCP region gate to the complete DRAM recovery mapping used by Nothing OS 4.1: four 1 KiB-rounded banks are checked with bounded 32-bit arithmetic, including the path where the DT flag is off but the handoff still advertises DRAM. The host gate reproduces the previous unchecked-span defect and proves the candidate leaves SCP/DVFS/sensorhub disabled. |
| `0100-vendor-scp-use-vfree-for-mailbox-tables.patch.vendor` | Fixes the live r161 probe-error crash by pairing the three `vzalloc` mailbox tables with `vfree`. This preserves fail-closed behavior when the bootloader SCP region-info handoff is absent or malformed; it does not enable SCP or sensorhub. |
