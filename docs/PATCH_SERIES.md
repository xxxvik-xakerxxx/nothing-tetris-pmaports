...
| `0084-drm-mediatek-bypass-MT6878-OVL-shadow.patch` | Adds the MT6878 `need_bypass_shadow` behavior from NothingOSS to mainline OVL data. Installed r143 proves the full route still stalls its first frame and starts OVL0/1/2 with bit 22 clear; r144 sets the bit in every `mtk_ovl_start()` lifecycle. CI and live validation are pending. |
