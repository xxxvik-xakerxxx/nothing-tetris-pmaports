...
| `0087-drm-mediatek-complete-MT6878-DSC-handoff.patch` | Fully writes the five registers from `mtk_ddp_insert_dsc_prim_mt6878()` and `C00/C0C` when the DRM route connects after DISP power/clock enable, clearing stale LK branches. Patch `0088` handles the atomic OVL start sequence. Live tracing on installed r147 proved that late selector or relay writes cannot recover an already stalled graph; r149 tests the authoritative cold-start/DPMS lifecycle. |
