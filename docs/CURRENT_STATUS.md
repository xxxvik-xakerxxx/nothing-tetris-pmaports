...
| Active pmaports source | `codex/next-hardware` | Installed image code is exact commit `fdeeda0`. Audio candidate `384f155` built kernel and device packages in CI `33522638794`; image creation then hit a stale assertion that still expected the deliberately deferred mono remap in static PulseAudio config. The following uninstalled source candidate fixes that CI contract and uses kernel `pkgrel=128` plus device `8-r5` to replace manual GNSS v050 with stock-derived v051. |
