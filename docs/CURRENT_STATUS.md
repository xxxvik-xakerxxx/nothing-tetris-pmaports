...
| Active pmaports source | `codex/next-hardware` at `267902b3b00e8e8403bd5898a1767ae56c412ff8` plus the pending r146 change | Installed r145 preserves the DSC parameter-load bit, full OVL chain and OVL shadow bypass. The next candidate changes only MT6878 cold-start ordering and remains off `main` until clean pixels and lifecycle gates pass. |
| Prepared display-order candidate | Pending CI, kernel package `6.18-r146` | Clean paired r145 disproved UUID mismatch and confirmed automatic USB SSH with zero failed units. DSC mode `0x10001` alone did not fix the frame. Reversible late reset-on-EOF and OVL-trigger probes produced no `FRAME_DONE`; OVL remains valid without ready and DSI input remains `1x1`. Patch `0085` configures MT6878 OVL/DSC/DSI and preserves the active scanout plane before enabling mutex/SOF, matching the B4.1 DRM v2 transaction boundary that cannot be reproduced by a late live write. |
...
| Display | Broken | Clean paired r145 boots without framebuffer fallback and retains automatic USB SSH. The parameter-load mode bit is now correct, but OVL0/1/2 still report valid without ready, DSC produces abnormal EOF without `FRAME_DONE`, and DSI input remains `1x1`. Reset-on-EOF and explicit OVL trigger live probes changed no counters and were fully reverted. | Build and clean-flash r146 with MT6878 components started before mutex/SOF. Require DSC `FRAME_DONE`, advancing counters, clean pixels and stable USB before promotion; then test brightness, blank/unblank, warm reboot and suspend/resume. |
