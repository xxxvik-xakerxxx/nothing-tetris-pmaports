...
| Installed native-display candidate | CI `34022276175`, pmaports `43bbc3d`, kernel `6.18.0 #135` | SHA-verified `super`/`userdata` clean-flashed and booted automatically with USB NCM/SSH and touch intact. The FIT has one native DTB and no simplefb. Patch `0074` worked: the power controller bound after the optional HWCCF `-EPERM`. DRM still did not register because display consumers referenced an unregistered nested DISP provider. Function-graph tracing reached `dev_pm_domain_attach()` but never entered `mtk_smi_larb_probe()`. |
| Staged source candidate | Kernel package `6.18-r135` in the working tree | Patch `0075` points larb0, mutex, OVL, DSC and DSI at `<&spm MT6878_POWER_DOMAIN_DISP>`, the onecell provider actually registered by SCPSYS and used by the vendor DT. The next gate is CI, clean flash and native DRM runtime evidence. |
...
| Display | Broken | Native-only CI `34022276175` clean-boots with USB SSH and touch and contains no framebuffer fallback. `MM_INFRA` now survives the optional HWCCF `-EPERM` and the power-controller binds. DRM remains absent because the display consumers use the nested `&disp` node as a provider, while SCPSYS registers only the `spm` onecell provider; live function-graph tracing proved failure in `dev_pm_domain_attach()` before SMI probe. | Build patch `0075`, clean-flash, then require larb/IOMMU and native DRM bind before pixel, brightness, blank/unblank, stability and suspend/resume tests. |
...
Native display is installed without any framebuffer fallback. The HWCCF failure
is fixed and the power controller binds; the remaining pre-DRM blocker is the
incorrect nested DISP provider reference. The r135 source candidate corrects
that one DT contract. The next GNSS gate is
