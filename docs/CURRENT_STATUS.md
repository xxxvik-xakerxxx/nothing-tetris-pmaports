...
| Active pmaports source | `codex/next-hardware` at `3dd12a1c64e4c4ca73eef9bec13540fc7cf52838` plus pending r148 changes | Installed r147 carries the integrated panel/DSI/DSC/OVL parity set. It remains off `main` until clean pixels and lifecycle gates pass. |
| Prepared native-display integration | Pending CI, kernel package `6.18-r148` | Patch `0087` fully normalizes the five vendor DSC handoff registers plus `C00/C0C` after DISP power/clock enable, then reapplies OVL force-relay after `OVL_EN`. Patch `0088` clears only `PQ_LOOP_CON[0]` at the B4.1 offset `0x2e0` after reset and before enable. The complete 88-patch series applies locally, overlay validation passes, and `mtk_disp_ovl.o` plus `mtk-mmsys.o` compile with LLVM. Runtime success is not claimed before a clean paired image test. |
