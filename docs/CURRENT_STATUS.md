...
| Staged source candidate | Kernel package `6.18-r140` in the working tree | Patch `0080` removes the unsupported one-revision allowlist. It retains the mandatory full three-byte transfer and logs the detected revision; board DT `compatible = "samsung,s6e8fc3x02"` remains the hardware selector, matching the official Nothing OS 4.1 driver's model. The next gate is static validation, CI, clean flash, panel enable and stable native pixels. |
...
| Display | Partial | Native-only CI `34091038734` boots without framebuffer fallback and retains USB SSH/touch. Mutex, OVL and DSI bind without Oops; native DRM registers `card0`, connected `DSI-1`, 1080x2400 and `mediatekdrmfb`. Patch `0079` obtains the complete panel ID `40 41 02`; only the unsupported `40 21 01` allowlist aborts preparation. | Build patch `0080`, clean-flash, require panel enable and stable pixels, then validate brightness, blank/unblank and suspend/resume. |
...
Installed r139 registers native DRM without Oops and reads the complete panel
ID `40 41 02`, but the early port rejects it against an unsupported one-revision
allowlist. The r140 source candidate removes that allowlist while preserving
the complete-transfer gate. The next GNSS gate is
