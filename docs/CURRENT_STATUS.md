...
| Prepared native-display integration | Pending fresh CI, kernel package `6.18-r149` | Patch `0086` now owns the complete atomic OVL start sequence: reset/INTSTA clear, masked `PQ_LOOP_CON[0]` clear at B4.1 offset `0x2e0`, `OVL_EN`, then force-relay. Patch `0087` normalizes the five vendor DSC handoff registers plus `C00/C0C` after DISP power/clock enable. CI runs `34446306149`, `34453008831` and `34453992035` exposed that a second patch touching `mtk_ovl_start()` was fragile in pmbootstrap prepare; that second hunk and standalone `0088` are removed. Runtime success is not claimed before successful CI and a clean paired image test. |
