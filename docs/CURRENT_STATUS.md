...
| --- | --- | --- |
| Installed pmaports image | `267902b3b00e8e8403bd5898a1767ae56c412ff8` | Kernel package `6.18-r145`, kernel `6.18.0 #146`; CI run `34223452817`; paired `super` and all 17 `userdata` chunks were clean-flashed. USB NCM/SSH, root UUIDs and system services return automatically, but pixels remain corrupt and no complete DSC frame is produced. |
...
| Active pmaports source | `codex/next-hardware` at `267902b3b00e8e8403bd5898a1767ae56c412ff8` plus the pending r146 change | Installed r145 preserves the DSC parameter-load bit, full OVL chain and OVL shadow bypass. The next candidate adds only the missing MT6878 DSC 1.1 version field and remains off `main` until clean pixels and lifecycle gates pass. |
| Prepared DSC-version candidate | Pending CI, kernel package `6.18-r146` | Clean paired r145 disproved UUID mismatch and confirmed automatic USB SSH with zero failed units. DSC mode `0x10001` alone did not fix the frame. Direct NothingOS review found a separate required DSC 1.1 field at `DSC+0x200[6:5]` that the native port never programs; patch `0085` now adds only that write. The superseded start-before-mutex experiment was cancelled before producing or installing an image because video-mode NothingOS ordering enables the mutex first. |
...
| Display | Broken | Clean paired r145 boots without framebuffer fallback and retains automatic USB SSH. The parameter-load mode bit is now correct, but OVL0/1/2 still report valid without ready, DSC produces abnormal EOF without `FRAME_DONE`, and DSI input remains `1x1`. Direct NothingOS review found the separate DSC 1.1 field at `DSC+0x200[6:5]` is never programmed by the native port. | Build and clean-flash r146 with only the missing DSC 1.1 field added. Require DSC `FRAME_DONE`, advancing counters, clean pixels and stable USB before promotion; then test brightness, blank/unblank, warm reboot and suspend/resume. |
