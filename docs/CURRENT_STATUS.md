...
| --- | --- | --- |
| Installed pmaports image | `1f8aef1` / CI `34455218621` | Kernel package `6.18-r149`, kernel `6.18.0 #150`. A paired clean flash boots automatically; USB NCM/SSH, touch, Phoc, backlight and the native connector return. Pixels remain corrupt and DSC reports abnormal EOF without a completed frame. |
...
| Active pmaports source | `codex/hardware-integration`, pending CI | Consolidates the installed r149 display baseline with fail-open USB identity, the greetd dconf fix, scoped audio controls, read-only GNSS diagnostics and compile-only charging, GPU, camera, sensor and modem boundaries. It remains off `main` until the native display and regression gates pass. |
| Prepared native-display integration | Pending fresh CI, kernel package `6.18-r151` | Installed r149 proves the complete vendor MMSYS selector values are present, but DSC remains stuck. The authoritative Tetris panel leaves `dsc_param_load_mode=0`; r149 incorrectly starts with `DSC_MODE=0x00010001`. Patch `0088` restores the vendor default `0x00000001`. A live write proved register access without losing USB, but cannot validate first-frame initialization; paired clean-image testing remains required. |
...
| Idle battery drain | Broken | Roughly half the battery was reported lost overnight. A live r147 capture found `dconf-service` at 5.1 GiB RSS plus 596 MiB swap and persistent CPU use because `/var/lib/greetd/.config/dconf` did not exist; Calls and Chatty retried failed writes continuously. A soft restart reclaimed the memory, but growth resumed until the directory was created. The next device package installs a tmpfiles rule for it. Wi-Fi and USB suspend costs remain unisolated. | Validate flat dconf RSS/CPU across reboot, then run physically unplugged screen-off A/B intervals with Wi-Fi associated and disabled. Record coulomb, suspend and wake/IRQ deltas. |
...
| Display | Broken | Installed r149 boots without framebuffer fallback and retains USB SSH, touch, Phoc and backlight. MMSYS routing matches Nothing OS, but OVL remains downstream-blocked and DSC reports `ABN_EOF` without `FRAME_DONE`. Register audit found the port forced `DSC_MODE=0x00010001` while the Tetris vendor configuration requires `0x00000001`. | Build and paired-clean-flash r151 with patch `0088`; require `FRAME_DONE`, advancing counters, clean pixels and stable USB before promotion, then test DPMS, brightness, warm reboot and suspend/resume. |
