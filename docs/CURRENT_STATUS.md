...
| Installed native-display candidate | CI `34017277889`, pmaports `67e7f9c`, kernel `6.18.0 #134` | Clean-flashed `super`/`userdata` boot automatically and restored USB NCM/SSH. The FIT contains only the native DTB and has no simplefb; simpledrm/sysfb are excluded. Runtime exposed no DRM card because `MM_INFRA` power-on stopped at an optional HWCCF call rejected by this firmware with `-EPERM`. Touch remained enumerated. |
| Staged source candidate | Kernel package `6.18-r134` in the working tree | Patch `0074` mirrors the MT6878 vendor contract narrowly: an observed `-EPERM` from the optional HWCCF infra hint no longer aborts the bounded hardware-voter handshake; other SMC errors remain fatal. The next gate is CI, clean flash and native DRM runtime evidence. |
...
| Display | Broken | Native-only CI `34017277889` clean-boots with USB SSH and touch, but creates no DRM card. Live reprobe proved the first failure: `MM_INFRA` aborts on optional HWCCF SMC `0x540` returning `-EPERM`. Public MT6878 vendor code calls the same hint but ignores its result before performing the bounded HWV handshake. | Build patch `0074`, clean-flash with r132 retained, then require MM_INFRA/DISP and native DRM bind before pixel, brightness, blank/unblank, stability and suspend/resume tests. |
...
Native display is installed without any framebuffer fallback, but is currently
blocked before DRM registration by the confirmed `MM_INFRA` HWCCF `-EPERM`.
The r134 source candidate addresses that one failure. The next GNSS gate is
