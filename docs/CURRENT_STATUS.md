...
| Prepared native-display integration | Pending CI, kernel package `6.18-r148` | Patch `0087` completes the vendor DSC insertion selector state and reapplies OVL force-relay after every reset. Patch `0088` initializes the MT6878 display crossbars and clears the OVL PQ loop after reset, matching the vendor cold-start sequence. The complete 88-patch series applies locally, overlay validation passes, and `mtk_disp_ovl.o` plus `mtk-mmsys.o` compile with LLVM 21. Runtime success is not claimed before a clean paired image test. |
