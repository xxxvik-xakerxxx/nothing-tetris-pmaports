...
| pmaports candidate | `codex/next-hardware` | `pkgrel=126`; first integrated CI exposed and stopped at a malformed compile-only camera patch before image generation; the corrected revision is not installed. |
...
| Rear/front cameras | Broken | No camera media pipeline or preview/capture. Torch channels work independently. | Corrected PD9302A init and an IMX882 raw-ID object are compile-only in `pkgrel=126`; no camera DT client or module is shipped/autoloaded. Sensor live identity, SENINF/ISP and the media graph remain. |
