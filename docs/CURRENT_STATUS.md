...
| Active pmaports source | `codex/next-hardware`, current working tree | The tree stages kernel package `6.18-r139` over installed image commit `8f2e5dc`. It remains off `main` until native pixels and display lifecycle pass without USB/touch regression. |
| Installed native-display candidate | CI `34084388144`, pmaports `8f2e5dc`, kernel package `6.18-r138`, kernel `6.18.0 #139` | SHA-verified `super`/`userdata` clean-flashed. USB NCM/SSH and `fts_ts event0` returned automatically with no legacy framebuffer, Oops or failed units. Mutex, OVL and DSI bind; `/dev/dri/card0`, connected `DSI-1`, mode `1080x2400` and `mediatekdrmfb` register. Patch `0078` removes the VM_DONE timeout and DSI IRQ 360 advances. The panel returns its default one-byte packet for the three-byte ID read, so prepare aborts, the connector remains disabled and no backlight is registered. |
| Staged source candidate | Kernel package `6.18-r139` in the working tree | Patch `0079` calls the standard MIPI DSI maximum-return-packet-size API before the three-byte panel ID read, matching the behavior required by the Nothing OS 4.1 MT6878 host without adding vendor policy to the generic host. The next gate is static validation, CI, clean flash, full panel initialization and stable native pixels. |
...
| Display | Partial | Native-only CI `34084388144` boots without framebuffer fallback and retains USB SSH/touch. Mutex, OVL and DSI bind without Oops; native DRM registers `card0`, connected `DSI-1`, 1080x2400 and `mediatekdrmfb`. Patch `0078` fixes the stale VM_DONE wait and DSI IRQ 360 advances to 34. The panel then returns one byte for its three-byte ID read, so prepare aborts before enable and backlight registration. | Build patch `0079`, clean-flash, require a three-byte ID response, complete panel initialization and stable pixels, then validate brightness, blank/unblank and suspend/resume. |
...
Installed r138 registers native DRM without Oops and completes command-mode DSI
IRQs, but panel preparation stops on its default one-byte return packet. The
r139 source candidate requests the required three-byte packet before reading
the panel ID. The next GNSS gate is
