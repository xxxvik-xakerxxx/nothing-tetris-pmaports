...
| Active pmaports source | `codex/next-hardware` at `3ea6876fb5031302464ff732cddc1319b627c147` plus the pending r147 integration | Installed r145 preserves the DSC parameter-load bit, full OVL chain and OVL shadow bypass. The next candidate combines the confirmed MT6878 native-display differences and remains off `main` until clean pixels and lifecycle gates pass. |
| Prepared native-display integration | Pending CI, kernel package `6.18-r147` | Patch `0085` programs DSC 1.1. Patch `0086` adds B4.1 panel generic/DCS packet selection, exact N4 D-PHY and horizontal timing, `TXRX_CTRL` video-mode parity, masked `PSCTRL` ownership, preserved LK `HSTX_CKL_WC`, measured DSI FIFO thresholds, explicit DSC SPR disable and OVL reset/INTSTA ordering. r146 CI was cancelled before artifacts or installation when the integration scope was expanded. |
...
| Display | Broken | Clean paired r145 boots without framebuffer fallback and retains automatic USB SSH, but OVL/DSC/DSI do not complete a valid physical frame. Source and register-dump comparison found multiple remaining B4.1/LK differences now collected in r147, including the high `PSCTRL` state, untouched `HSTX_CKL_WC=0x10000`, exact N4 timings and FIFO ownership. | Build and clean-flash matched r147 `super` and `userdata`. Require DSC `FRAME_DONE`, advancing counters, clean pixels and stable USB before promotion; then test brightness, blank/unblank, warm reboot and suspend/resume. |
