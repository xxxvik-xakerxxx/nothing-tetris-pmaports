...
| Staged source candidate | Kernel package `6.18-r133` in the working tree | Native MMSYS clocks/routing, DISP/MM power, OVL, DSC, DSI at the vendor-confirmed 1140 Mbps lane rate, MIPI PHY, bounded display DMA/IOMMU and the Samsung panel are packaged as patches `0056`-`0073`. The only FIT DTB removes simplefb, and simpledrm/sysfb are excluded from the kernel. The 18-patch series strict-applies and the native DTB passes host DTC; CI and device runtime remain. |
...
| Display | Untested | Installed r132/simpledrm is clean with U-Boot `60bcf22`, confirming the earlier artifacts were in framebuffer handoff. Packaged r133 replaces it with native MMSYS/OVL/DSC/DSI/PHY/panel and bounded SMI/IOMMU DMA. Its only FIT DTB removes simplefb. The panel state that skipped `backlight_enable()` and its zero default brightness were fixed. | Build in CI, flash `super` with the r132 rollback retained, then require native DRM bind, pixels, brightness, touch, blank/unblank, stability, suspend/resume and uninterrupted USB SSH. |
...
Native display is enabled only in the pending r133 source candidate. The next
GNSS gate is userspace protocol integration and a real position fix, followed
by cold-start and lifecycle validation.
