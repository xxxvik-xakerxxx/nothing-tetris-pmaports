...
| Staged source candidate | Kernel package `6.18-r133` in the working tree | Native MMSYS clocks/routing, DISP/MM power, OVL, DSC, DSI, MIPI PHY, bounded display DMA/IOMMU and the Samsung panel are packaged as patches `0056`-`0072`. The only FIT DTB removes simplefb, and simpledrm/sysfb are excluded from the kernel. The 17-patch series strict-applies and targeted LLVM 21 objects/native DTB pass; CI and device runtime remain. |
