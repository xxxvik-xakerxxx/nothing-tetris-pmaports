...
| Prepared DSC-mode candidate | Pending CI, kernel package `6.18-r145` | A paired r132 `super`/`userdata` boot restored kernel `#130` and USB SSH, allowing the same probe to capture a working LK register reference. All DSC geometry, buffer and PPS registers match r144, but working LK has `DSC_MODE=0x00010001` and status `0x1`; r144 has `0x00000001` and abnormal-EOF status `0x9`. The candidate sets bit 16 during initial MT6878 DSC programming. A reversible late live write changed DSC state but could not validate initial sequencing, so no functional claim is made before a clean candidate boot. |
...
| Display | Broken | Installed r144 boots without framebuffer fallback and retains automatic USB SSH, Phoc, touch, panel ID and backlight. A paired r132 reference boot and paired r144 restore both passed USB SSH. Register comparison isolates the only static DSC programming mismatch: working LK retains mode `0x10001`, while r144 writes `0x1`; r144 also has abnormal EOF instead of the reference status `0x1`. Other DSC geometry/PPS values match exactly. | Build and clean-flash r145 with the mode bit applied during initial configuration. Require DSC `FRAME_DONE`, advancing counters, clean pixels and stable USB before promotion; then test brightness, blank/unblank, warm reboot and suspend/resume. |
