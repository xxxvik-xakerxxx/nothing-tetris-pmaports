...
| GNSS | Partial | On #128 with U-Boot `b76e47e`, v050 creates both `gpsdl` nodes and completes a bounded link0 open/close without connectivity regressions. Ioctl 23 returns `EFAULT` because v050 links a non-ATF stub. Stock-derived Tetris property and `vendor_dlkm` trees both select and load v051. | Compile/package v051 as the only GNSS profile, then clean-boot transport/boot-info tests before MVCD/MNL, position and lifecycle work. |
