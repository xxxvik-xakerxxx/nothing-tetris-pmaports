...
| Installed native-display candidate | CI `34050069460`, pmaports `e415ad5`, kernel `6.18.0 #138` | SHA-verified `super`/`userdata` clean-flashed. USB NCM/SSH and `fts_ts event0` returned automatically with no legacy framebuffer or Oops. Patch `0077` works: mutex, OVL and DSI bind; `/dev/dri/card0`, enabled/connected `DSI-1`, mode `1080x2400` and `mediatekdrmfb` register. Panel initialization stops before its first DCS write because stale U-Boot video mode causes an impossible initial VM_DONE wait; DSI IRQ count remains zero. |
| Staged source candidate | Kernel package `6.18-r138` in the working tree | Patch `0078` resets only MT6878 DSI to command mode during power-on, before panel prepare. The requested video mode remains selected and started later by normal atomic enable. The next gate is CI, clean flash, successful CMD_DONE transfers and stable native pixels. |
...
| Display | Partial | Native-only CI `34050069460` boots without framebuffer fallback and retains USB SSH/touch. Mutex, OVL and DSI bind without Oops; native DRM registers `card0`, connected/enabled `DSI-1`, 1080x2400 and `mediatekdrmfb`. The first panel command fails because stale bootloader video mode makes mainline wait for VM_DONE before the new pipeline has emitted a frame. | Build patch `0078`, clean-flash, require successful CMD_DONE panel initialization and stable pixels, then validate brightness, blank/unblank and suspend/resume. |
...
Installed r137 registers native DRM without Oops, but panel DCS initialization
hits stale bootloader video mode. The r138 source candidate resets command mode
before panel prepare. The next GNSS gate is
