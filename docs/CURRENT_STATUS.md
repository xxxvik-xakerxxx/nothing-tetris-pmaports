...
| Active pmaports source | `codex/next-hardware`, current working tree | The tree stages kernel package `6.18-r141` over installed image commit `e641138`. It remains off `main` until native pixels and display lifecycle pass without USB/touch regression. |
...
| Staged source candidate | Kernel package `6.18-r141` in the working tree | Patch `0081` selects the DSC DSI stream type and derives `PS_WC=1080` plus DSI buffer width `360` from the two 540-byte slices, matching Nothing OS 4.1. CI `34107810232` caught a missing complete-type DSC include; the current source adds the standard DRM DSC header. Next gate: rebuilt CI, clean flash, stable physical pixels, then brightness/blank/suspend lifecycle while USB and touch remain available. |
...
| Display | Partial | Native-only r140 boots without framebuffer fallback and retains USB SSH/touch. Native DRM, Phoc, panel ID `40 41 02`, backlight and blank/unblank all work, but physical output is stable stripes while `/dev/fb0` contains structured pixels. Source comparison found the host emits uncompressed framing (`PS_SEL=3`, `PS_WC=3240`, width 1080) for a DSC stream that requires (`PS_SEL=5`, `PS_WC=1080`, width 360). | Build patch `0081`, clean-flash and require stable physical pixels; then validate brightness, blank/unblank and suspend/resume. |
...
Installed r140 registers and enables native DRM without Oops, reads panel ID
`40 41 02`, starts Phoc and preserves USB/touch. Physical output is striped
because the mainline DSI host frames the compressed DSC line as uncompressed
RGB888. The r141 source candidate ports the official MT6878 DSC packet word
count, stream type and buffer width. The next GNSS gate is
