...
is in `local/live-logs/20260912T170639Z-172.16.42.1-audit`. The user visually
confirmed good display output on this clean image.

The clean node check shows touch as `fts_ts` on `/dev/input/event0`, keys and
RT6010 haptics in `/proc/bus/input/devices`, Wi-Fi as `wlan0`, powered BlueZ
`hci0`, and USB Type-C/gauge telemetry. It also shows the remaining blockers:
IIO contains only PMIC ADC devices, not accel/gyro/proximity/light; `/dev/dri`
has `card0` only and no render node; no `/dev/video*` or `/dev/media*` camera
nodes exist; ModemManager reports no modem. Wi-Fi is present but not associated
on this audit, and package index refresh produced transient DNS errors, so
routed Wi-Fi/DNS is not confirmed on this clean image.
...
The display promotion candidate at 63a21acc0dcd6139af02e2c8294010571331e0dc
fixes the greetd dconf ownership mismatch by depending on `greetd` before the
device package runs and resolving the packaged `greetd` UID/GID at install time
instead of baking one builder-local numeric owner. The previous r12 image build
failed because the post-install script ran before `greetd` existed in the
target rootfs; device r13 fixes that ordering. The integrated branch carries
the same install-time owner repair as device r10.

CI 34689206699 for `codex/display-main-promotion`
63a21acf5161707b2c24d30bec93324b15d025be produced install artifacts that
passed manifest verification. The downloaded ZIP SHA256 was
18b54f76ee5457ec270303671cfba8dfa187189a685542fb3632bd144037785c and the
manifest required U-Boot 60bcf22fdc0a94526424db59fc7640298ea8f0dd. The test
phone had a newer diagnostic loader in `lk_a`, so `lk_a` was restored to the
verified 60bcf22 recovery image before flashing the CI `super` and `userdata`
artifacts. After reboot the phone left fastboot and exposed the expected
postmarketOS CDC-NCM USB gadget (`18d1:d001`, serial `postmarketOS`). macOS did
not create the NCM host interface while `IOConsoleLocked = Yes`, so SSH and USB
transfer regression remain pending on this exact clean display image. The user
visually confirmed that the display output is good. This keeps native display
at Partial until SSH, touch, DPMS/brightness, warm reboot, suspend/resume and
repeat cold-start gates pass.
...
rootfs greeter-owner tests pass. CI 34689206699 completed, and its install
image artifact was hash-verified and flashed after restoring the required
60bcf22 loader. Main has not moved. The user confirmed visually good output on
that clean image, but SSH/USB transfer, touch, cold-repeat and lifecycle checks
remain pending because macOS did not attach the NCM host interface while the
console was locked.
...
| Idle battery drain | Broken | Roughly half the battery was reported lost overnight. A live r147 capture found `dconf-service` at 5.1 GiB RSS plus 596 MiB swap and persistent CPU use because `/var/lib/greetd/.config/dconf` did not exist; Calls and Chatty retried failed writes continuously. A soft restart reclaimed the memory, but growth resumed until the directory was created. Device package r10 now ships the private dconf directory directly, repairs ownership from target `/etc/passwd`, and the overlay validator rejects regressions. Wi-Fi and USB suspend costs remain unisolated. | Build and clean-install r10, validate flat dconf RSS/CPU across reboot, then run physically unplugged screen-off A/B intervals with Wi-Fi associated and disabled. Record coulomb, suspend and wake/IRQ deltas. |
...
| Display | Partial | Installed r155 continues to expose native `/sys/class/drm/card0/card0-DSI-1`; prior clean install and user inspection confirmed visually good output, and the frame-end synchronized buffer update removed the visible redraw flicker in live testing. CI 34689206699 for the display promotion candidate was hash-verified and clean-flashed after restoring the required 60bcf22 loader; the phone left fastboot, exposed the expected postmarketOS CDC-NCM gadget and the user visually confirmed good output. The panel still exposes only a fixed 60 Hz mode and the session has no render node, so perceived slowness is likely outside the display scanout fix. | Unlock/replug host USB to restore NCM on the clean display image, then retest SSH, USB transfer, touch, DPMS/brightness, warm reboot and suspend/resume before main promotion. |
