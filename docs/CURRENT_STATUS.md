...
| Prepared native-display integration | Pending fresh CI, kernel package `6.18-r149` | Patch `0087` normalizes the five vendor DSC handoff registers plus `C00/C0C` after DISP power/clock enable. Patch `0088` atomically clears only `PQ_LOOP_CON[0]` at the B4.1 offset `0x2e0`, enables OVL, then applies force-relay. CI runs `34446306149` and `34453008831` exposed a fragile split hunk before compilation; the consolidated complete series now applies to a fresh pinned tree locally. Runtime success is not claimed before successful CI and a clean paired image test. |
