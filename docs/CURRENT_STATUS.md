...
| Active pmaports source | `codex/next-hardware` | Installed image code is exact commit `fdeeda0`. GNSS v051 candidate `fb93b54` reached a Linux 6.18 `of_device_id` header blocker in CI `33540592686`. Commit `3f87a7a` fixed that boundary; CI `33853670112` then built kernel `6.18-r128` and the device package successfully before image validation rejected a stale mono-remap string. No install artifact was uploaded. The next candidate corrects that assertion and uses `6.18-r129` for the expanded compile/static-only hardware boundaries; no new runtime support is claimed until CI and clean-device gates pass. |
