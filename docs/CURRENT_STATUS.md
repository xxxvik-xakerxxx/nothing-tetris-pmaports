...
| Staged source candidate | `codex/next-hardware`, kernel `pkgrel=131`, device `8-r6` | BC1.2 compile-only integration passed CI `33880650257` at `ab97ff8`. The next source adds a fully disabled stock-derived main-IMX882 DT fixture and greeter-only PulseAudio autostart suppression. Hardware additions have no runtime path. Combined CI and installation evidence are pending, so the installed image remains the baseline. |
