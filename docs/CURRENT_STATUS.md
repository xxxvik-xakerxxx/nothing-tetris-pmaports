...
| Active pmaports source | `codex/next-hardware`, current working tree | Kernel package `6.18-r142` fixes the remaining DSC RC threshold encoding over installed r141. It remains off `main` until native pixels and display lifecycle pass without USB/touch regression. |
...
| Display | Partial | Installed r141 boots without framebuffer fallback and retains automatic USB SSH/touch. Native DRM, Phoc, panel ID `40 41 02` and backlight are enabled; OVL and DSI IRQs advance. Patch `0081` fixes compressed framing and removes moving noise, but output remains a stable pale frame with a blue vertical line. Source audit found patch `0062` scales the already encoded DRM RC thresholds a second time. | Build and clean-flash r142 with patch `0082`, then require clean pixels before brightness, blank/unblank, warm reboot and suspend/resume. |
...
Installed r141 registers and enables native DRM without Oops, reads panel ID
...
`0082` removes a second `>> 6` applied to RC threshold bytes that the DRM helper
already stores in PPS form. Physical pixel quality remains the next display gate. The next GNSS gate is
