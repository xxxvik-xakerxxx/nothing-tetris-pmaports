...
| Display | Broken | Clean `980c566` shows a stable fbcon, but Phoc produces physical noise or black while `/dev/fb0` contains a clean logical frame. Legacy KMS also booted black and was rolled back. Historical native r40 bound the MT6878 pipeline and created fb0; OVL frame-start IRQ bit 14 removed the recorded vblank timeouts, but the complete native patch set is not yet in the active package. | CI and device-test r132 damage-limited MMIO fallback. In parallel reconstruct a separate native DDP/DSC/DSI/panel DTB, preserving simplefb recovery and USB SSH. |
