...
| 8 | Native display | The legacy framebuffer is retired from the shipped DTB. Installed r144 binds the full OVL0/1/2, DSC and DSI graph, applies OVL shadow bypass and retains USB/touch, but its first frame still aborts: OVL positions stop at `15/5/2`, DSC has abnormal EOF without completion and DSI input remains `1x1`. Exact vendor panel/PPS/routes/buffer values are present; bounded live tests have ruled out reset, constant source, start delay/order, single-shot mutex, SMI-ID and HSTX as standalone fixes. | Boot the retained clean r132/LK framebuffer only with physical fastboot recovery available, capture the same register trace, and patch the first measured native-versus-reference mismatch. Then require clean OVL0/1/2 flow, DSC completion, advancing DSI counters, clean KMS pixels, stable USB, 60/120 Hz modes, brightness, blank/unblank, sustained touch, suspend/resume and repeated boots. |
