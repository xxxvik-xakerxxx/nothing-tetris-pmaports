...
| 8 | Native display | The legacy framebuffer is retired from the shipped DTB. Installed native r142 binds DRM, panel, OVL0, DSC and DSI, starts Phoc, retains touch/USB and drives backlight, but shows a stable pale frame with one blue line and DSC abnormal EOF. Correct RC thresholds made no visual change. Runtime registers and NothingOSS prove the hardware route must include OVL1/OVL2 and the full PQ-bypass crossbar chain; r143 implements that exact graph and compiles. | First require clean OVL0/1/2 flow, DSC completion, DSI counters and USB SSH on r143. Then require clean KMS pixels, 60/120 Hz modes, brightness, blank/unblank, sustained touch, suspend/resume and repeated boots. |
