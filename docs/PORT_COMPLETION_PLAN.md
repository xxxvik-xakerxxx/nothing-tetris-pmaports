...
| 8 | Native display | The legacy framebuffer is retired from the shipped DTB. Installed r143 binds the full OVL0/1/2, DSC and DSI graph and retains USB/touch, but its first frame times out with OVL downstream-blocked and DSC input stuck at `1x1`. Selector, mutex, reset and chunk-size live tests did not recover it. NothingOSS requires OVL shadow bypass; r143 does not set it and r144 now carries the missing lifecycle behavior. | Require clean OVL0/1/2 flow, DSC completion, advancing DSI counters and USB SSH on r144 before physical judgement. Then require clean KMS pixels, 60/120 Hz modes, brightness, blank/unblank, sustained touch, suspend/resume and repeated boots. |
