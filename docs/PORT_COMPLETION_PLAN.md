...
| 8 | Native display | Simpledrm is usable but not complete: r132 had persistent artifacts with U-Boot `b76e47e`, then clean Phoc output on two boots after only U-Boot changed to `60bcf22`. Historical native r40 bound OVL/COLOR/CCORR/AAL/GAMMA/DSI and created fb0; OVL frame-start IRQ bit 14 removed vblank timeouts. Panel, OVL, bounded MMSYS/mutex routing and MT6878 DSC sidecars compile; finish clocks/power, complete crossbar, DSI/PHY and an opt-in DTB/FIT before changing the default path. | KMS framebuffer, 60/120 Hz modes, brightness, blank/unblank, sustained touch, suspend/resume and fallback boot path all work while USB SSH remains stable. |
