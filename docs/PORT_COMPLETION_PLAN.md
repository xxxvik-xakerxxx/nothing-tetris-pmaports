...
| Installed device candidate | CI image from `c2b19a9` | Kernel package `6.18-r132` and device `8-r6` were clean-flashed from CI `33954042399`. Linux reaches userspace with valid NCM and touch enumeration, but physical Phoc output still contains artifacts. The r132 inherited-simpledrm experiment is rejected and remains off `main`. |
