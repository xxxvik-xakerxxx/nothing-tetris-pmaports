...
| 8 | Native display | The legacy framebuffer is retired from the shipped DTB. Paired r132 and r144 boots produced directly comparable register captures while USB SSH remained recoverable. All captured DSC geometry/PPS values match, but working LK retains `DSC_MODE=0x10001` with status `0x1`; native r144 writes `0x1` and reports abnormal EOF. The r145 candidate applies that proven mode difference during initial DSC setup. | Pass CI and clean-flash paired r145 images. Require clean OVL0/1/2 flow, DSC completion, advancing DSI counters, clean KMS pixels, stable USB, 60/120 Hz modes, brightness, blank/unblank, sustained touch, suspend/resume and repeated boots. |
