...
validate_compile_only_boundaries() {
	...
	grep -Fq '0085-drm-mediatek-program-MT6878-DSC-version.patch' \
		"$kernel_apkbuild"
	grep -Fq '#define DISP_REG_DSC_VERSION_MT6878' \
		"$kernel_pkg/0085-drm-mediatek-program-MT6878-DSC-version.patch"
	grep -Fq 'DSC_VERSION_1_1, &priv->cmdq_reg,' \
		"$kernel_pkg/0085-drm-mediatek-program-MT6878-DSC-version.patch"
	...
	if grep -El '^[[:space:]]*(pd9302a|tetris-camera-audit|panel-samsung-s6e8fc3x02|ccci_md_all|ccci_all|ccci_ccif|ccci_dpmaif|ccci_modem|ap_md_mem|dpmaif|mt6375-bc12-decode)[[:space:]]*$' \
		"$device_pkg"/*.conf >/dev/null 2>&1; then
