...
validate_compile_only_boundaries() {
	...
	if grep -El '^[[:space:]]*(panthor|pd9302a|tetris-camera-audit|panel-samsung-s6e8fc3x02|ccci_md_all|ccci_all|ccci_ccif|ccci_modem|ap_md_mem|mt6375-bc12-decode)[[:space:]]*$' \
		"$device_pkg"/*.conf >/dev/null 2>&1; then
