...
| Display | Native DSI/panel | Broken | Clean paired r145 is installed with automatic USB SSH/touch and no framebuffer fallback. Preserving the MT6878 DSC parameter-load bit did not complete a frame: OVL remains valid without ready, DSC reports abnormal EOF without frame done, and DSI input remains `1x1`. Direct NothingOS review found that the native port never programs the separate DSC 1.1 field at `DSC+0x200[6:5]`; r146 adds only that vendor-backed write. No fix is claimed until CI, clean flash and physical pixels pass. |
