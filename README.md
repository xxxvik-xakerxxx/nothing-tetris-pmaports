...
| Sensors | Rotation/accelerometer | Partial | On r167, real HF samples reach standard iio-sensor-proxy through its native MediaTek backend. User verified portrait and both landscape orientations after the Tetris mount-matrix correction. Gyro/magnetic samples exist but calibration, warm boot and suspend remain unverified. See `docs/SENSOR_DESKTOP_INTEGRATION.md`. |
| Sensors | Ambient light/proximity | Partial | User verified automatic brightness and proximity blank/unblank during a local dummy call. Actual light, proximity and backlight changes were recorded. Brightness smoothness, clean packaged installation and lifecycle remain open; this is not GSM call verification. Backend is included in the next CI image with guarded opt-in startup. |
