...
| Kernel version | `6.18` (installed native-display `pkgrel=144`, kernel `6.18.0 #145`) |
...
| Display | Native DSI/panel | Broken | Native-only r144 is installed with automatic USB SSH/touch and no framebuffer fallback. DRM, Phoc, panel ID `40 41 02`, backlight and OVL shadow bypass initialize; the corruption pattern changes, but no frame completes. OVL0/1/2 stop at `15/5/2`, DSC reports repeated abnormal EOF, and DSI remains at `1x1` with underrun/input-unfinished. Exact vendor panel/PPS/routes/buffer values are present; bounded live tests ruled out reset, constant source, start timing, mutex mode, SMI-ID and HSTX as standalone fixes. The next gate is a register comparison with the retained working r132/LK framebuffer boot while physical fastboot recovery is available. |
