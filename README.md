...
| Kernel version | `6.18` (installed native-display `pkgrel=144`, kernel `6.18.0 #145`; r145 candidate prepared) |
...
| Display | Native DSI/panel | Broken | Native-only r144 is installed with automatic USB SSH/touch and no framebuffer fallback. A clean paired r132 reference boot proved that every DSC geometry/PPS register matches r144 except `DSC_MODE`: working LK retains `0x00010001`, while native r144 writes `0x00000001` and reports abnormal EOF. The r145 candidate preserves the MT6878 parameter-load bit during initial DSC configuration. This is evidence-backed but not a fix claim until CI, clean flash, DSC completion and physical pixels pass. |
