...
| Kernel version | `6.18` (installed native-display `pkgrel=145`, kernel `6.18.0 #146`; r146 candidate prepared) |
...
| Display | Native DSI/panel | Broken | Clean paired r145 is installed with automatic USB SSH/touch and no framebuffer fallback. Preserving the MT6878 DSC parameter-load bit did not complete a frame: OVL remains valid without ready, DSC reports abnormal EOF without frame done, and DSI input remains `1x1`. Live reset and OVL-trigger probes were reverted after producing no progress. The r146 candidate matches the B4.1 transaction boundary by configuring MT6878 OVL/DSC/DSI and the active scanout plane before enabling mutex/SOF. No fix is claimed until CI, clean flash and physical pixels pass. |
