...
| Display | Native DSI/panel | Partial | Native-only r140 clean-boots kernel `#141` with USB SSH/touch and no framebuffer fallback. DRM, Phoc, panel ID `40 41 02`, native backlight and blank/unblank work, but physical output is striped because the host uses uncompressed DSI framing for the DSC payload. r141 ports the official MT6878 compressed stream type, 1080-byte line payload and 360-pixel DSI buffer width. |
