...
| Display | Native DSI/panel | Partial | Native-only r141 is clean-flashed and boots kernel `#142` with automatic USB SSH/touch and no framebuffer fallback. DRM, Phoc, panel ID `40 41 02`, native backlight and advancing OVL/DSI IRQs work. Correct DSC DSI framing removes the moving stripe noise, but the panel shows a uniform pale frame with a blue vertical line because the encoder RC thresholds are incorrectly shifted twice. r142 fixes only that PPS register error; clean physical pixels and lifecycle remain promotion gates. |
